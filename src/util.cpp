...
const char * const BITCOIN_CONF_FILENAME = "levocoin.conf";
const char * const BITCOIN_PID_FILENAME = "levocoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "levocoin" is a composite category enabling all Levocoin-related debug output
                if(ptrCategory->count(std::string("levocoin"))) {
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Levocoin";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty levocoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override levocoin.conf
