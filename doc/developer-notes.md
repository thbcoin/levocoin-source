...
Levocoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 5393 for connections and services them.
...
However, in open source software such as Levocoin Core, where everyone uses
...
pay attention to for reviewers of Levocoin Core code.

General Levocoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Levocoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Levocoin Core so that they can be integrated
