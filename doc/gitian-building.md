...
*Setup instructions for a Gitian build of Levocoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Levocoin
...
- [Building Levocoin Core](#building-levocoin-core)
...
Clone the git repositories for Levocoin Core and Gitian.
...
in the Levocoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Levocoin Core
----------------

To build Levocoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Levocoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/levocoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/levocoinpay/levocoin
...
For example:
```bash
URL=https://github.com/crowning-/levocoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit levocoin=${COMMIT} --url levocoin=${URL} ../levocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit levocoin=${COMMIT} --url levocoin=${URL} ../levocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit levocoin=${COMMIT} --url levocoin=${URL} ../levocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the levocoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../levocoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/levocoin
SIGPATH=/some/root/path/levocoin-detached-sigs

./bin/gbuild --url levocoin=${BTCPATH},signature=${SIGPATH} ../levocoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/levocoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/levocoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/levocoin-osx-build.assert
```
...
[levocoin/gitian.sigs](https://github.com/levocoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
